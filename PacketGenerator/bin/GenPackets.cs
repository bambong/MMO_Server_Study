...
class C_Chat : IPacket
{
	...
	public ushort Protocol { get => (ushort)PacketID.C_Chat; }
    ...
    public ArraySegment<byte> Write()
    {
        ...
        count += sizeof(ushort);
        var chatLen = (ushort) Encoding.Unicode.GetBytes(this.chat, 0, this.chat.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		success &= BitConverter.TryWriteBytes(s.Slice(count, chatLen), chatLen);
		count += sizeof(ushort);
		count += chatLen;
...
class S_Chat : IPacket
{
	...
	public ushort Protocol { get => (ushort)PacketID.S_Chat; }
